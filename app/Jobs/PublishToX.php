...
class PublishToX implements ShouldQueue
{
    ...
    public function handle(SocialMediaService $socialService): void
    {
        try {
            ...
            if (!$connection) {
                Log::warning("No X (Twitter) connection found for user {$this->post->user->id}");
                return;
            }

            $socialService->publishToX($this->post, $connection);

            Log::info("Published post {$this->post->id} to X (Twitter)");
        } catch (\Exception $e) {
